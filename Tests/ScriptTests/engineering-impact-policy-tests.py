...
"""
    ...
    def test_packaging_reuses_the_verified_release_build_products(self):
        ...
        self.assertIn('target_architecture="$(uname -m)"', release_build)
        self.assertIn("-sdk macosx", release_build)
        self.assertIn('ARCHS="$target_architecture"', release_build)
        self.assertIn('target_architecture="$(uname -m)"', package_script)
        self.assertIn("-sdk macosx", package_script)
        ...
        self.assertIn('macos_sdk_path_sha256=', receipt_script)
        self.assertIn('macos_sdk_version=', receipt_script)
        self.assertIn('macos_sdk_build_version=', receipt_script)
